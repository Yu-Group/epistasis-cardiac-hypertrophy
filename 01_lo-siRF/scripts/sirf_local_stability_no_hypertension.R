...
RESULTS_DIR <- file.path("..", "results", "no_htn")
...
htn_ids <- readRDS(file.path("..", "data", "htn_ids.rds"))
...
if (!file.exists(file.path(save_path, "irf_interaction_fit.Rdata"))) {
  ...
  # remove individuals with HTN
  keep_train_idx <- !(rownames(out$geno_train) %in% htn_ids)
  keep_test_ids <- !(rownames(out$geno_test) %in% htn_ids)
  out$geno_train <- out$geno_train[keep_train_idx, ]
  out$pheno_train <- out$pheno_train[keep_train_idx]
  out$geno_test <- out$geno_test[keep_test_ids, ]
  out$pheno_test <- out$pheno_test[keep_test_ids]
